...
# Journalists to include for avg engaged minutes metric
AVG_MINUTES_JOURNALISTS = {
    'Lena Geller',
...
def print_date_range_report(df, top_n, metrics, ignore_authors, file_metadata):
    ...
    for metric, col_name in metrics.items():
        # For avg_engaged_minutes, filter to specific journalists only
        if metric == 'avg_engaged_minutes':
            filtered_stats = author_stats[author_stats['Author'].isin(AVG_MINUTES_JOURNALISTS)]
            sorted_df = filtered_stats.sort_values(metric, ascending=False).head(top_n)
        else:
            # Sort by metric for all other metrics
...
def print_monthly_report(df, top_n, metrics, ignore_authors, file_metadata):
    ...
    for metric, col_name in metrics.items():
        ...
        for year_month in sorted(processed_df['Year-Month'].unique()):
            ...
            # For avg_engaged_minutes, filter to specific journalists only
            if metric == 'avg_engaged_minutes':
                month_stats = month_stats[month_stats['Author'].isin(AVG_MINUTES_JOURNALISTS)]
