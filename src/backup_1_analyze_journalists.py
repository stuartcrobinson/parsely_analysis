...
def analyze_journalists(input_csv, start_date, top_n, ignore_authors):
    ...
    for metric_name, column_name in metrics.items():
        ...
        for month in months:
            ...
            # Track winners (anyone in top position)
            ...
            # Format output with first names
            formatted = []
            for author, value in top_authors.items():
                first_name = get_first_name(author)
                formatted.append(f"{first_name} ({int(value)})")
        ...
        if month_winners:
            click.echo(f"\nMonths Won:")
            for author, count in sorted(month_winners.items(), key=lambda x: x[1], reverse=True):
                first_name = get_first_name(author)
                click.echo(f"{first_name}: {count}")
        ...
        for i, (author, value) in enumerate(top_overall.items()):
            click.echo(f"{i+1}. {author}: {int(value)}")
