...
def aggregate_journalist_metrics(df, ignored_authors):
    ...
    for _, row in df.iterrows():
        ...
        for author in authors:
            ...
            # Use full name for aggregate data
...
def print_results(journalist_data, journalist_articles, monthly_data, monthly_articles, 
                  top_n_aggregate, top_n_monthly, input_file, filter_date, ignored_authors):
    ...
    print(f"Top N (aggregate): {top_n_aggregate}")
    print(f"Top N (monthly): {top_n_monthly}")
    ...
    for metric in METRIC_COLUMNS.keys():
        ...
        # Get top performers
        top_performers = get_top_n_overall(journalist_data, journalist_articles, metric, top_n_aggregate)
        
        base_header = f"TOP {top_n_aggregate} JOURNALISTS BY {metric.upper().replace('_', ' ')}"
...
@click.option('--top-n-aggregate', type=int, default=5, help='Number of top journalists to show in aggregate rankings')
@click.option('--top-n-monthly', type=int, default=5, help='Number of top journalists to show in monthly rankings')
...
def main(input_file, top_n_aggregate, top_n_monthly, ignore_authors, after_date):
    """Analyze journalist performance metrics from Parsely data."""
