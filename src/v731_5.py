...
def aggregate_by_author(df, metrics, start_date=None, end_date=None):
    ...
    # For publications, we need to count articles published within the date range
    if start_date and end_date and 'Publish date' in df.columns:
        ...
        pub_counts = published_in_range.groupby('Author')['URL'].count().reset_index()
        pub_counts.columns = ['Author', 'publications']
